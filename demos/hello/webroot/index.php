...
{
	static $ROOT = null;
	static private $_config = [ SITE=>[], MODE=>[], REWRITE=>[], DB=>[], MM=>[], MM_OPTIONS=>[], SERVICE=>[], MODULE=>[], VIEW_CACHE_PATH=>'/tmp/_views/', LANG_PATH=>'langs/', LOG_PATH=>'logs/', DISABLE_VIEW_CACHE=>false, ENABLE_MSVC_CROSS=>false, ONREWRITE=>[], ONIN=>[], ONOUT=>[], ONERROR=>[] ];
	static private $_site_paths = [];
	static private $_site_view_tags = [];
	static private $_site_view_replace = [];
	static private $_auto_classes = [];
	static private $_site = null;
	static private $_path = null;
	static private $_exists_site_path = false;
	static private $_is_service = false;
	static private $_all_errors = [];
	static private $_response_code = 200;
	static private $_response_headers = [];
	static $_call_logs = [];
	static private $_is_succeed = false;
	static private $_display_errors = false;
	static private $_default_view_tag_replaces = [
		'<!--\s*@(.+?)\s*-->',
		'<!--\s*#(.+?)\s*-->',
		'<!--\s*/(.*?)-->',
		'<([\w\.]*?:\w+)\s*(.*?)>',
	...
	static private $_view_tag_replaces = [
	...
	static private function _loadSite( $site )
	{
		...
		$check_config_files = [$config_file, $user_config_file];
		if( $site )
		{
...
// 		MSVC::_setCache( $config_file, [MSVC::$_config, MSVC::$_site_paths, MSVC::$_site_view_replace, MSVC::$_site_view_tags], $check_config_files );
	...
	static private function _makeError( $message )
	{
		...
		else if( !MSVC::$_is_service && MSVC::$_display_errors )
		{
			echo nl2br( "\n".$message );
		}
	}
	...
	{
		if( $errno == E_DEPRECATED || $errno == E_USER_DEPRECATED || $errno == E_STRICT || ( $errno == E_NOTICE && (strstr( $errstr, 'ndefined' ) || strstr( $errstr, 'Uninitialized' )) ) ) return true;
...
// 	}
	...
	static function init( $root_path )
	{
		...
		{
			// 设置错误处理函数
			set_error_handler( array('MSVC', '_errorHandler'), E_ALL );
// 			set_exception_handler( array('MSVC', '_exceptionHandler'));
			register_shutdown_function(array('MSVC', '_shutdownHandler'));
			...
			if( MSVC::$_config['LOG_PATH'][0] != '/' ) MSVC::$_config['LOG_PATH'] = MSVC::$ROOT.MSVC::$_config['LOG_PATH'];
			if( DIRECTORY_SEPARATOR == '\\' ) MSVC::$_config['VIEW_CACHE_PATH'] = 'c:/_views/';

			// 注册自动加载
			spl_autoload_register( ['MSVC', '_autoload'] );
...
// 							$_POST = json_decode( $json, true );
			...
			if( is_string( $output ) )
			{
				if( strpos( MSVC::$_path, '.html' ) !== false )
			...
			if( $mode[0] != 'T' )
			{
				...
				MSVC::log( [ $server_info, $_GET, $_POST, $_COOKIE, $_SESSION, $_FILES, MSVC::$_call_logs ], 'access' );
// MSVC::tt( 'finish' );
				return $result;
			}
		}
	...
	{
		// 搜索控制器
		$search_sites = MSVC::$_exists_site_path ? [MSVC::$_site] : [];
		$search_sites[] = '';
		// TODO 增加 modules
...
//echo "[$type] [$tmp_site] $tmp_path <br/>\n";
	...
	static public function callController( $path )
	{
		...
		// 初始化应用系统路径
		$tmp_paths = explode( '	', trim( str_replace( '/', '	', $path ) ) );	// 路径
		if( !$tmp_paths || !$tmp_paths[0] ) $tmp_paths = ['index'];	// 默认访问 index 类
		if( strstr( $tmp_paths[0], '.' ) ) $tmp_paths = ['index',$tmp_paths[0]];	// 第一段中有 . 直接将这个作为控制器的方法
...
// 		}
		...
		$method = str_replace( ['.html','.php'], '', $method );	// 如果是 .html 去掉 .html 就是控制器的方法
...
// 		echo "$class_path | $class_name | $method<br/>\n";
	...
	/**
	 * 调用服务
	 * @param string $service_name 服务名称 package.class.method
	 * @param array $args 服务的参数，k-v数组
	 */
	static private $_sercice_call_counting = 0;
...
// 			header( 'HTTP/1.1 500 Internal Server Error' );
	...
	static private function _doCall( $class_name, $method, $args )
	{
		...
		{
			if( $is_controller )
				$tmp_result = call_user_func( [$class_obj, '_willCall'], $method );
			else
				$tmp_result = call_user_func( [$class_obj, '_willCall'], $method, $args );
			if( $tmp_result === false )
			{
		...
		if( $is_controller )
			$result = call_user_func( [ $class_obj, $method ] );
		else
			$result = call_user_func( [ $class_obj, $method ], $args );

		// 后置调用（调用 _didCall 进行验证） 返回 false 表示拒绝执行， true 表示通过检查， 返回其他内容表示直接处理不需要再进行调用（相当于魔术方法）
		if( method_exists( $class_obj, '_didCall' ) )
		{
			if( $is_controller )
				$tmp_result = call_user_func( [$class_obj, '_didCall'], $method, $result );
			else
				$tmp_result = call_user_func( [$class_obj, '_didCall'], $method, $args, $result );
	...
	static function getModel( $model_name )
	{
		...
		if( !$m )
		{
			...
			{
				$m = new $class_name( $db, $real_table, [$dsn, $user, $pwd, $options], MSVC::$_config['MM'], MSVC::$_config['MM_OPTIONS'] );
			}
			else
			{
				// 使用默认的Model类
				$m = new Model( $db, $real_table, [$dsn, $user, $pwd, $options], MSVC::$_config['MM'], MSVC::$_config['MM_OPTIONS'] );
			}
	...
	static private $_langs = [];
	static private $_cur_lang_set = null;
	static function getLang( $text, $langs=null )
	{
		...
		if( !$langs )
		{
			if( !MSVC::$_cur_lang_set )
			{
				...
				if( !$al ) return $text;
				MSVC::$_cur_lang_set = $al[2]=='-' ? [ substr( $al, 0, 5 ), substr( $al, 0, 2 ) ] : [ substr( $al, 0, 2 ) ];
			}
...
class Model
{
	...
	function __construct( $db, $table, $db_info, $mm_servers, $mm_options )
	{
		...
		$this->_write_pdo_info = [$dsn, $user, $pwd];
		$this->_read_pdo_infos = [ [$dsn, $user, $pwd] ];
		if( strstr( $dsn, '|' ) )
		{
// 			mysql:host=w.db:3306|w.db:3307;dbname=GY_AUTH
			...
			if( count($m) == 4 )
			{
				...
				if( count($hosts) >= 2 )
				{
					$this->_write_pdo_info = [$m[1].$hosts[0].$m[3], $user, $pwd];
					$this->_read_pdo_infos = [];
					for( $i=count($hosts)-1; $i>=1; $i-- ) $this->_read_pdo_infos[] = [$m[1].$hosts[$i].$m[3], $user, $pwd];
				}
			}
		}
		if( !$options )
	...
	private function _makeWheres( $wheres, $args )
	{
		...
		{
			$where = $wheres;
			$where_args = is_string($args) ? [$args] : $args;
		}
	...
	function query( $sql, $args=[] )
	{
		$this->_clear_query();
		$this->_full_sql = $sql;
		$this->_full_sql_args = is_string($args) ? [$args] : $args;
		return $this;
	}
	...
	private function _exec( $sql, $args=[], $return_q=false )
	{
		try
		{
			...
			MSVC::$_call_logs[] = ['M',$sql,microtime(true)-$start_time,$args,''];
			return $return_q ? $q : $isok;
		}
		catch( Exception $ex )
		{
			MSVC::$_call_logs[] = ['M',$sql,microtime(true)-$start_time,$args,$ex->getMessage()];
	...
	private function _doSelect( $fields=null, $make_first=false )
	{
		...
		// 使用缓存
		$cache_sql_key = preg_replace( '/[^a-zA-Z0-9\x{4e00}-\x{9fff}]/u', '', str_replace( ['select','from','where','orderby','groupby','leftjoin'], '', strtolower( $this->_last_sql.join( '', array_values($this->_last_sql_args) ) ) ) );
...
abstract class Task
{
	...
	function _willCall()
	{
		...
		if( $this->info['start'] && $now_time < $this->info['start'] ) $this->log( "[NORUN]	$now_time < ".$this->info['start'] ) || die( "[NORUN]	$now_time < ".$this->info['start']."\n" );
		if( $this->info['stop'] && $now_time > $this->info['stop'] ) $this->log( "[NORUN]	$now_time > ".$this->info['stop'] ) || die( "[NORUN]	$now_time > ".$this->info['stop']."\n" );
		$running_num = trim( `ps ax | grep {$this->task_name} | grep -v grep | wc -l` );
		if( $running_num > 1 ) $this->log( "[RUNNING]	$running_num {$this->task_name} is running" ) || die( "[RUNNING]	$running_num {$this->task_name} is running\n" );
...
/**
 * 启动 MSVC 框架
 * @param string $root_path	入口文件相对于站点的路径，或一个绝对路径表示站点根目录，多数情况都是 ..
 */
 ...
 * @return array
<string , any>，固定返回的数据： error=>int 0 表示没有错误，非0表示错误代码，message
错误信息或者其他想传达的信息 */ function S( $service_name, $args=[] ) { $start_time =
microtime( true ); $r = MSVC::callService( $service_name, $args );
MSVC::$_call_logs[] =
['S',$service_name,microtime(true)-$start_time,$args,$r['error']];
return $r; } /** * 调用 View 显示一个页面 * @param string $view_file View文件的相对路径
* @param array $data 要跟界面结合的数据，如果是列表使用数组，多重嵌套的列表使用嵌套的数组 * @return string
要数据的页面数据 */ function V( $view_file=null, $data=[] ) { $start_time =
microtime( true ); $r = MSVC::display( $view_file, $data );
MSVC::$_call_logs[] = ['V',$view_file,microtime(true)-$start_time];
return $r; } /** * 调用 Controller 处理一个页面，可以在控制其中递归调用实现页面跳转 * @param
string $path 页面的相对路径 * @return string or array 返回字符串表示输出页面，返回对象以 JSON
格式输出数据 */ function C( $path ) { $start_time = microtime( true ); $r =
MSVC::callController( $path ); MSVC::$_call_logs[] =
['C',$path,microtime(true)-$start_time]; return $r; } /** *
调用语言处理函数，将一段文本转换成对应的语言文本 * @param string $text 要转换的文本 * @return string
对应的语言文本 */ function L( $text, $langs=null ) { return MSVC::getLang(
$text, $langs ); } /** * 记录错误信息 * @param string $message 错误信息 */
function E( $message ) { MSVC::log( $message, 'error' ); } /** * 记录错误信息
* @param string $message 错误信息 */ function I( $message ) { MSVC::log(
$message, 'info' ); } /** * 记录错误信息 * @param string $message 错误信息 */
function D( $message ) { MSVC::log( $message, 'debug' ); } /** *
构造一个成功的标准结果，error 为 0 * @param array $obj 要返回的数据 * @return array
加上标准输出参数 error、message 之后的结果（Key-Value格式） */ function OK( $obj=null ) {
if( !is_array( $obj ) ) $obj = []; $obj['error'] = 0; $obj['message'] =
''; return $obj; } /** * 构造一个处理失败的标准结果，error 如果没有指定 默认为 -1 * @param
array $obj 要返回的数据 * @return array 加上标准输出参数 error、message
之后的结果（Key-Value格式） */ function FAILED( $message, $error=-1, $obj=null )
{ if( !is_array( $obj ) ) $obj = []; $obj['error'] = $error != -1 ?
$error : -1; $obj['message'] = $message; return $obj; } /** * 生成 in sql
语句，对所有成员进行 mysql_escape_string * @param array $values 要生成 in sql 语句的数据 *
@return array 加上标准输出参数 error、message 之后的结果（Key-Value格式） */ function IN(
$values ) { if( !is_array( $values ) ) return '()'; foreach( $values as
&$v ) $v = mysql_escape_string( $v ); return "('" . join( "','", $values
) . "')"; } /* ------------------------------ 启动处理
------------------------------- */ if( PHP_SAPI == 'cli' ) { if( $argc <
2 ) die( "Usage:\n php index.php TASK package.taskclass args\n php
index.php TEST package.class.testaction args\n" ); $mode = $argv[1]; if(
in_array( $mode, ['TEST','TASK'] ) ) { MSVC::init( '..' ); echo
MSVC::start( $mode ); } } else { MSVC( '..' ); } 